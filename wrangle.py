...
def acquire():
    ...
    if os.path.isfile(file_name):
        ...
        query = 'SELECT bedroomcnt, bathroomcnt, calculatedfinishedsquarefeet, taxvaluedollarcnt, yearbuilt, taxamount, fips, parcelid FROM properties_2017 JOIN propertylandusetype USING (propertylandusetypeid) WHERE propertylandusedesc = "Single Family Residential"'
...
def acquire_sec():
    file_name = 'zillow_sec.csv'
    if os.path.isfile(file_name):
        return pd.read_csv(file_name)
    else:
        query = 'SELECT latitude, longitude, transactiondate, lotsizesquarefeet, parcelid FROM properties_2017 JOIN propertylandusetype USING (propertylandusetypeid) JOIN predictions_2017 USING (parcelid) WHERE propertylandusedesc = "Single Family Residential" AND transactiondate BETWEEN "2017-01-01" AND "2017-12-31"'
        connection = get_connection('zillow')
        df = pd.read_sql(query, connection)
        df.to_csv('zillow_sec.csv', index=False)
        return df
...
def prep():
    ...
    df = df.rename(columns={'bedroomcnt': 'beds', 'bathroomcnt': 'baths', 'calculatedfinishedsquarefeet': 'square_feet', 'taxvaluedollarcnt' : 'value', 'yearbuilt': 'year_built', 'taxamount':'tax_amount', 'fips': 'county'})
    ...
    # I've been wanting to drop this from the start
